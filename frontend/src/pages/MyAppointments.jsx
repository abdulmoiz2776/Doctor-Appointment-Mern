...
const MyAppointments = () => {
  ...
  const cancelAppointment=async(appointmentId)=>{
    try {
      const {data}=await axios.post(`${backendUrl}/api/auth/cancel-appointment`,{appointmentId},{headers:{Authorization:`Bearer ${token}`}})
      if(data.success){
        toast(data.message)
        getAppointmentData()
        getDoctorsData()
  ...
  const payOnline=async(appointmentId)=>{
    try {
      const {data}=await axios.post(`${backendUrl}/api/auth/pay-online`,{appointmentId},{headers:{Authorization:`Bearer ${token}`}})
      if(data.success){
        toast(data.message)
        getAppointmentData()
      }else{
        ...
        {appointments.map((item,index)=>(
          ...
          <div className='grid grid-cols-[1fr_2fr] gap-6 sm:flex py-2 border-b' key={index}>
            ...
            <div className='flex-start text-sm text-zinc-600'>
              ...
              <p  className='text-sm mt-1'>Date & time: <span className='text-sm text-neutral-700 font-medium'>{slotDateFormatted(item.slotDate)}, {item.slotTime}</span></p>
            </div>
            ...
            <div className='flex flex-col gap-2 justify-center'>
              ...
              {!item.cancelled && !item.payment && !item.isCompleted &&<button onClick={()=>payOnline(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-indigo-600 hover:text-white transition-all duration-300'>
                Pay Online
              </button>}
